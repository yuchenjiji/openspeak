...
class AzureSTTService implements STTService {
  ...
  Future<void> _startRecording(String language) async {
    try {
      ...
      _tempFilePath = "${dir.path}/openspeak_stt_${DateTime.now().millisecondsSinceEpoch}.wav";
      ...
      _partialTimer = Timer.periodic(const Duration(milliseconds: 600), (_) {
        ...
        _controller?.add(STTResult(text: indicator, isFinal: false, confidence: 0.0));
      });
    } catch (e) {
    ...
    try {
      ...
      if (file != null && await file.exists()) {
        ...
        if (text != null && text.isNotEmpty) {
          _controller?.add(STTResult(text: text, isFinal: true, confidence: 0.95));
        }
        // Cleanup temp file
        await file.delete();
      }
